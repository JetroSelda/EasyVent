...
import { NotebookPen, Users, TrendingDown, TrendingUp } from "lucide-react";
import { useEffect, useState } from "react";

const TotalProviders = ({ selectedFilter, filter }) => {
  ...
  const initiateData = (userData) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/stats/totalProviders.php`, {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
      .then(({ data }) => {
        const { list, delta } = filter(data?.users ?? []);
