...
const PrivacySettings = ({ settings = {}, refresh }) => {
  ...
  const handleChange = (value) => {
    setFormState((prev) => ({ ...prev, policy: value }));
  };

  const handleSubmit = async () => {
    if (isLoading) return;
    setIsLoading(true);
    const { policy } = currState;
    ...
    fetch(`${import.meta.env.VITE_API_URL}/settings/updatePolicy.php`, {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
    ...
    <Card>
      <CardContent>
        <div className="flex justify-between items-center mb-5">
          <div className="semi-bold">Policy Page</div>
        ...
        <div className="py-3 grid gap-2">
          <TextEditor disabled={isLoading || !enabledForm} defaultValue={currState.policy} onChange={handleChange} />
        </div>
      </CardContent>
    </Card>
  )
