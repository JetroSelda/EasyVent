...
import { Circle, Ellipsis, FileInput, Mail, PhilippinePeso, Phone, Smartphone, Timer, User, Users } from "lucide-react";
...
const RejectBookingForm = ({ defaultValue = {}, refetch }) => {
  ...
  const handleSubmit = (event) => {
    ...
    formData.append("name", [defaultValue.personal_name, defaultValue.last_name].join(" "));
...
const PaymentForm = ({ defaultValue, refetch }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [isRejecting, setIsRejecting] = useState(false);

  console.log("DefaultValue", defaultValue);

  const method = JSON.parse(defaultValue?.payment_method ?? "{}");
  const receipt = JSON.parse(defaultValue?.payment_receipt ?? "{}");
  ...
  const handleSubmit = async (event) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/booking/confirmPayment.php`, {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
  ...
  const handleReject = () => {
    if (isRejecting || isLoading) return;
    setIsRejecting(true);
    ...
    fetch(`${import.meta.env.VITE_API_URL}/booking/rejectPayment.php`, {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid gap-3 pt-5">
        ...
        <Card className="p-0 shadow-none" onClick={() => {
          window.open(`${import.meta.env.VITE_API_URL}/uploads/${receipt.url}`, "_blank")
        }}>
          <CardContent className="flex h-[4rem] px-3 items-center gap-2 cursor-pointer">
            ...
            <p className="w-[15rem]">
              <h4 className="font-semibold w-[90%] text-nowrap text-ellipsis overflow-hidden">{receipt.name}</h4>
              <p className="text-gray-500 text-[0.9rem] w-[90%] text-nowrap text-ellipsis overflow-hidden">{receipt.type} file</p>
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent className="px-3">
            <div className="flex mb-2 items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={method?.type !== "LANDBANK" ? "h-[3rem]" : "h-[1.5rem] mt-[0.5rem] mb-[0.5rem]"}>
                  <img src={PAYMENTS_LOGO[method?.type]} className="w-full h-full" />
                </div>
              </div>
            </div>
            ...
            <div className="grid mb-2 px-2">
              ...
              <div className="text-gray-500">{method?.account_name}</div>
            </div>
        ...
        <div className="flex items-center justify-end gap-3">
          <Button type="button" variant="outline" onClick={handleReject} disabled={isLoading || isRejecting}>
            {isRejecting && <Spinner />} Reject Payment
          </Button>
          <Button disabled={isLoading || isRejecting}>{isLoading && <Spinner />} Confirm Payment</Button>
        </div>
      </div>
    </form>
  )
  ...
  return (
    <Card className="p-0 rounded-md">
          ...
          <TableBody>
            {bookingList.map((booking) => (
              <TableRow key={booking.invoice}>
                ...
                <TableCell>{formatDate(booking.schedule)}</TableCell>
      ...
      <Dialog open={!!paymentState} onOpenChange={setPaymentState}>
        <DialogContent className="sm:max-w-[355px] max-h-[85vh] overflow-auto">
          {!!paymentState && (
            <PaymentForm defaultValue={paymentState} refetch={() => {
              fetchBookingList();
