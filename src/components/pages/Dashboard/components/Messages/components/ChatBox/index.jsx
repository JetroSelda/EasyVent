...
const ChatBox = ({ selectedChat }) => {
  ...
  const sendMessage = (event) => {
    ...
    formData.append("receiverId", String(parsedUser.id) === String(selectedChat.id_user1) ? selectedChat.id_user2 : selectedChat.id_user1);
  ...
  const initiateMessages = (id) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/chat/messages.php`, {
      ...
      .then(({ data = {} }) => {
        ...
        setTimeout(() => initiateMessages(selectedChat.id), 1500);
      });
  }

  useEffect(() => {
    if (!selectedChat.id) return;
    initiateMessages(selectedChat.id);
